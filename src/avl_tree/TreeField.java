...
class TreeField<T> extends Pane {
    ...
    void drawTree(TreeAVL.Node node, double x, double y, double k) {

        if (node != null) {
            addVertex((T) node.getKey(), x, y); // что это
            if (node.getLeft() != null) {
                addLine(x + VERTEX_RADIUS, y + VERTEX_RADIUS,            //cтык
                        x + VERTEX_RADIUS - k, y + VERTEX_RADIUS + 60);  //стык
                addVertex((T) node.getKey(), x, y); // и это
